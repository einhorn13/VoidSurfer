...
export class GameDirector {
    ...
    purchaseShip(shipId) {
        ...
        if (!playerStats || !oldCargo) return false;

        const newShipData = this.dataManager.getShipData(shipId);
        const cost = newShipData?.cost ?? 9999999;
        
        if (playerStats.credits < cost) return false;

        if (oldCargo.currentMass > newShipData.cargoCapacity) {
            eventBus.emit('notification', { text: `Cannot transfer cargo: new ship has smaller cargo hold`, type: 'danger' });
            return false;
        }
        ...
        if (oldAmmo) {
            for (const [type, count] of oldAmmo.ammo.entries()) {
                 if (newAmmo.hasOwnProperty(type)) {
                    newAmmo[type] = Math.min(newShipData.ammo[type], (newAmmo[type] || 0) + count);
                }
            }
        }
        this.gameStateManager.playerState.ammo = newAmmo;
        this.gameStateManager.playerState.cargo = Object.fromEntries(oldCargo.items);
    ...
    spawnNewNpc(shipId, position, objective, factionOverride = null) {
        const options = { position };
        if (factionOverride) {
            options.faction = factionOverride;
        }
    ...
    createObjective(objective, spawnPosition) {
        if (objective.type === 'TRADE_RUN_STATION') {
            ...
            if (stationTransform) {
                const dockable = this.ecsWorld.getComponent(this.worldManager.stationEntityId, 'DockableComponent');
                const approachDistance = (dockable?.dockingRadius || 200) * 2;
                
                const stationUp = new THREE.Vector3(0, 1, 0).applyQuaternion(stationTransform.rotation);
                const approachPoint = stationTransform.position.clone().add(stationUp.multiplyScalar(approachDistance));
                const loiterTime = THREE.MathUtils.randFloat(30, 60);

                const distToStation = spawnPosition.distanceTo(stationTransform.position);
                if (distToStation < approachDistance * 1.5) {
                    const awayVector = new THREE.Vector3().subVectors(spawnPosition, stationTransform.position).normalize();
                    const targetPos = spawnPosition.clone().add(awayVector.multiplyScalar(approachDistance * 4));
                    return { type: 'NAVIGATE_TO', targetPosition: targetPos };
                } else {
                    return { type: 'NAVIGATE_TO', targetPosition: approachPoint, loiterTime };
                }
            }
        }
