...
export class HitResolverSystem extends System {
    ...
    _resolveHit(hitEvent) {
        ...
        // temporary debug collision
        if (sourceId && this.world.getComponent(sourceId, 'ProjectileComponent')) {
            const projCollision = this.world.getComponent(sourceId, 'CollisionComponent');
            ...
            if (projCollision && targetCollision && targetTransform) {
                const projRadius = projCollision.boundingSphere.radius;
                const targetRadius = targetCollision.boundingSphere.radius;
                const effectiveTestRadius = targetRadius + projRadius;
                const actualImpactDistance = targetTransform.position.distanceTo(impactPoint);

                console.log(
                    `%c--- PROJECTILE HIT DEBUG ---
                    - Projectile Radius: ${projRadius.toFixed(4)}
                    - Target Radius (visual): ${targetRadius.toFixed(4)}
                    - Effective Test Radius: ${effectiveTestRadius.toFixed(4)}
                    - Actual Impact Distance from Target Center: ${actualImpactDistance.toFixed(4)}`,
                    'color: orange'
                );
            }
        }
        // end temporary debug collision
        ...
        if (!sourceFaction) return; // Safety check
        ...
        if (sourceWeaponData.explosionRadius > 0) {
            this._applyAoeDamage(impactPoint, sourceWeaponData, sourceFaction);
        }
    ...
    _resolvePhysicalCollision(sourceData, targetId, impactPoint) {
        ...
        if (this.world.getComponent(idA, 'HealthComponent')) {
            this.world.publish('damage', { targetId: idA, amount: damage, impactPoint, impactNormal: collisionNormal.clone().negate() });
        }
        if (this.world.getComponent(idB, 'HealthComponent')) {
            this.world.publish('damage', { targetId: idB, amount: damage, impactPoint, impactNormal: collisionNormal });
        }
    ...
    _applyAoeDamage(center, weaponData, sourceFaction) {
